...
def train(file_name_and_path, test_train_ratio, log_file_path, max_steps, train_batch_size, test_batch_size, learning_rate, dropout_rate, model_dir):
  ...
  x_4d = utilities.reshape(x, [-1, 1000, 3, 1])  

  conv1 = utilities.conv_layer(x_4d, [3,3,1,32], padding='SAME', name_suffix='1')

  pool1 = utilities.pool_layer(conv1, shape=[1,8,1,1], strides=[1,8,1,1],  name_suffix='1')

  conv2 = utilities.conv_layer(pool1, [3,3,8,160], padding='SAME', name_suffix='2')

  pool2 = utilities.pool_layer(conv2, shape=[1,5,1,1], strides=[1,5,1,1], name_suffix='1')

  conv3 = utilities.conv_layer(pool2, [1,3,16,160], padding='VALID', name_suffix='3')  

  flatten = utilities.reshape(conv3, [-1, 4000])
  ...
  y2 = utilities.reshape(utilities.fc_layer(dropped, 1000, num_states_out2*num_cols_out2, layer_name='softmax_2', act=tf.nn.softmax), [-1, num_cols_out2, num_states_out2])
