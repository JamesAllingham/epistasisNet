...
class PoolConvModel(model.Model):
    ...
    def __init__(self, x, y1_, y2_, learning_rate):
        ...
        with tf.name_scope('softmax_2'):
            fc_layer = utilities.fc_layer(dropped, int(flatten_size/4), num_cols_out2*num_states_out2, layer_name='identity', act=tf.identity)
        ...
        self._accuracy2 = utilities.calculate_snp_accuracy(output2, y2_, cut_off_prob=0.5, name_suffix='2')

        # find the top predicted snps
        self._epi_snps, self._count = utilities.predict_snps(output2, cut_off_prob=0.5)
